# dashboard_app/dash_apps.py (Açı Hesaplama Düzeltmesi)
...
DEFAULT_UI_INITIAL_ANGLE = 135 # Fiziksel başlangıç açısı
DEFAULT_UI_SCAN_ARC = 270      # Taranacak toplam yay (derece)
...
if app is None:
    print(f"!!!!!! [PID:{os.getpid()}] dash_apps.py: CRITICAL - 'app' could not be initialized. DjangoDash instantiation failed. !!!!!!")
...
control_panel = dbc.Card([
    ...
    dbc.CardBody([
        ...
        dbc.InputGroup([dbc.InputGroupText("Başl. Açı (°)", style={"width": "120px"}), # Bu, sensor_script'e --initial_goto_angle olarak gidecek
                        dbc.Input(id="initial-angle-input", type="number", value=DEFAULT_UI_INITIAL_ANGLE, min=-179,
                                  max=179, step=5)], className="mb-2"),
        # <<< DEĞİŞİKLİK: "Bitiş Açısı" yerine "Tarama Açısı (Toplam Derece)" >>>
        dbc.InputGroup([dbc.InputGroupText("Tarama Açısı (°)", style={"width": "120px"}),
                        dbc.Input(id="scan-arc-input", type="number", value=DEFAULT_UI_SCAN_ARC, min=10,
                                  max=340, step=10)], className="mb-2"), # ID: scan-arc-input
...
def get_latest_scan_id_from_db(conn_param=None):
    ...
    if conn_to_use:
        ...
        except (sqlite3.Error, pd.io.sql.DatabaseError) as e: print(f"Son tarama ID DB hatası: {e}"); latest_id = None
        except Exception as e: print(f"Son tarama ID genel hata: {e}"); latest_id = None
        finally:
            if internal_conn and conn_to_use:
                try: conn_to_use.close()
                except sqlite3.Error as e: print(f"DB kapatma hatası (get_latest_scan_id): {e}")
    return latest_id
...
if app:
    ...
    def fetch_data_for_store(n_intervals):
        ...
        try:
            ...
            else: output_data['error'] = f"Tarama ID {latest_id} için bilgi bulunamadı."
            ...
            return output_data
        except (sqlite3.Error, pd.io.sql.DatabaseError) as e: output_data['error'] = f"Veri çekme DB hatası: {str(e)[:100]}"; return output_data
        except Exception as e: output_data['error'] = f"Genel veri çekme hatası: {str(e)[:100]}"; return output_data
        finally:
            if conn:
                try: conn.close()
                except sqlite3.Error as e_close: print(f"DB kapatma hatası (fetch_data_for_store): {e_close}")

    # <<< BAŞLATMA BUTONU CALLBACK'İ GÜNCELLENDİ >>>
    @app.callback(Output('scan-status-message', 'children'),
                  [Input('start-scan-button', 'n_clicks')],
                  [State('initial-angle-input', 'value'), # UI'dan gelen mutlak başlangıç açısı
                   State('scan-arc-input', 'value'),      # UI'dan gelen toplam tarama yayı
    ...
    def handle_start_scan_script(n_clicks_start, initial_angle_val, scan_arc_val, step_angle_val):
        ...
        # sensor_script.py'ye gönderilecek olan "bitiş açısını" hesapla
        # sensor_script.py, initial > final ise sola döneceğini anlar.
        final_a_conceptual = initial_a_conceptual - scan_arc

        # Parametre doğrulama
        if not (-179 <= initial_a_conceptual <= 179): return dbc.Alert("Başlangıç açısı -179 ile 179 arasında olmalı!", color="danger", duration=4000)
        if not (10 <= scan_arc <= 340): return dbc.Alert("Tarama açısı 10 ile 340 derece arasında olmalı!", color="danger", duration=4000)
        if not (1 <= step_a <= 45): return dbc.Alert("Adım açısı 1 ile 45 arasında olmalı!", color="danger", duration=4000)

        # Hesaplanan final_a_conceptual'ın sensor_script için makul olup olmadığını kontrol etmeye gerek yok,
        # sensor_script kendi içinde normalize_angle_0_360 ile bunu yönetir.
        ...
        if current_pid and is_process_running(current_pid): return dbc.Alert(f"Betik zaten çalışıyor (PID: {current_pid}).", color="warning", duration=4000)
        if os.path.exists(LOCK_FILE_PATH_FOR_DASH):
            ...
            except OSError as e: return dbc.Alert(f"Kalıntı kilit/PID silinemedi: {e}.", color="danger", duration=4000)
        try:
            python_executable = sys.executable
            if not os.path.exists(SENSOR_SCRIPT_PATH): return dbc.Alert(f"Betik bulunamadı: {SENSOR_SCRIPT_PATH}", color="danger", duration=4000)

            cmd = [python_executable, SENSOR_SCRIPT_PATH,
                   "--initial_goto_angle", str(initial_a_conceptual),
                   "--scan_end_angle", str(final_a_conceptual), # Hesaplanan bitiş açısı
                   "--scan_step_angle", str(step_a)]
            print(f"Dash: Sensör betiği başlatılıyor: {' '.join(cmd)}")
            process = subprocess.Popen(cmd, start_new_session=(os.name == 'posix'), creationflags=(subprocess.CREATE_NEW_PROCESS_GROUP if os.name != 'posix' else 0), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            time.sleep(2.5); stdout_data, stderr_data = "", ""
            try:
                if process.poll() is not None:
                    stdout_data_bytes, stderr_data_bytes = process.communicate(timeout=1); stdout_data = stdout_data_bytes.decode(errors='ignore'); stderr_data = stderr_data_bytes.decode(errors='ignore')
            ...
            if os.path.exists(PID_FILE_PATH_FOR_DASH):
                ...
                try:
                    ...
                    if new_pid and is_process_running(new_pid): return dbc.Alert(f"Sensör okumaları başladı (PID: {new_pid}).", color="success", duration=4000)
                    else:
                ...
                except Exception as e: return dbc.Alert(f"PID dosyası okunurken hata: {e}", color="warning", duration=4000)
            else:
        ...
        except Exception as e: return dbc.Alert(f"Sensör başlatılırken hata: {str(e)}", color="danger", duration=6000)

    # Diğer tüm callback'ler (update_all_graphs_from_store, handle_stop_scan_script, vb.)
    # önceki (dashboard_app_final_v1) versiyonundaki gibi kalabilir.
    # Sadece `update_all_graphs_from_store` ve `update_environment_estimation`
    # callback'lerinin adlarını değiştirdim (fonksiyon isimleri farklı olabilir,
    # önemli olan decorator'daki Output/Input/State tanımlarıdır).
    # Bu callback'ler zaten dcc.Store'dan veri okuyordu.

    @app.callback(
        [Output('scan-map-graph', 'figure'), Output('polar-graph', 'figure'), Output('time-series-graph', 'figure')],
        Input('live-scan-data-store', 'data')
    )
    def update_all_graphs_from_store(stored_data):
        try:
            empty_fig_layout = go.Layout(title_text='Veri Bekleniyor...', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(248,248,248,0.95)'); error_fig_layout = go.Layout(title_text='Grafik Yüklenemedi', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(248,248,248,0.95)'); empty_fig = go.Figure(layout=empty_fig_layout); error_fig = go.Figure(layout=error_fig_layout)
            if not stored_data: return empty_fig, empty_fig, empty_fig
            db_error = stored_data.get('error'); scan_id_for_title = stored_data.get('scan_id', "N/A"); scan_status_for_title = stored_data.get('info', {}).get('status', 'Bilinmiyor'); title_suffix_base = f"(ID: {scan_id_for_title}, Durum: {scan_status_for_title})"
            if db_error: error_fig.update_layout(title_text=f'Veri Hatası: {str(db_error)[:50]} {title_suffix_base}'); return error_fig, error_fig, error_fig
            if not stored_data.get('points') and not db_error : empty_fig.update_layout(title_text=f'Nokta Verisi Yok {title_suffix_base}'); return empty_fig, empty_fig, empty_fig
            df_points = pd.DataFrame(stored_data.get('points', [])); title_suffix = title_suffix_base; max_plot_dist = 200.0
            fig_map_layout = go.Layout(title_text='2D Harita ' + title_suffix, xaxis_title="Yatay (cm)", yaxis_title="İleri (cm)", yaxis_scaleanchor="x", yaxis_scaleratio=1, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(248,248,248,0.95)'); fig_map = go.Figure(layout=fig_map_layout)
            if not df_points.empty:
                try:
                    ...
                    if not df_valid_map.empty: fig_map.add_trace(go.Scatter(x=df_valid_map['y_cm'], y=df_valid_map['x_cm'], mode='lines+markers', name='Sınır', marker=dict(size=5, color=df_valid_map['mesafe_cm'], colorscale='Viridis', showscale=False), line=dict(color='dodgerblue'))); polygon_plot_x = [0.0] + list(df_valid_map['y_cm']) + [0.0]; polygon_plot_y = [0.0] + list(df_valid_map['x_cm']) + [0.0]; fig_map.add_trace(go.Scatter(x=polygon_plot_x, y=polygon_plot_y, fill="toself", fillcolor='rgba(0,176,246,0.2)', line=dict(color='rgba(255,255,255,0)'), showlegend=False))
                except KeyError as ke: fig_map.update_layout(title_text='Harita Veri Formatı Hatası')
                fig_map.add_trace(go.Scatter(x=[0], y=[0], mode='markers', marker=dict(size=10, symbol='diamond', color='red'), name='Sensör'))
            fig_polar_layout = go.Layout(title_text='Polar Grafik ' + title_suffix, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(248,248,248,0.95)'); fig_polar = go.Figure(layout=fig_polar_layout)
            if not df_points.empty:
                try:
                    df_valid_polar = df_points[(df_points['mesafe_cm'] > 0.1) & (df_points['mesafe_cm'] < max_plot_dist) & df_points['angle_deg'].notna()].copy()
                    if not df_valid_polar.empty: fig_polar.add_trace(go.Scatterpolar(r=df_valid_polar['mesafe_cm'], theta=df_valid_polar['angle_deg'], mode='lines+markers', name='Mesafe', marker=dict(color=df_valid_polar['mesafe_cm'], colorscale='Viridis', showscale=True, colorbar_title_text="Mesafe(cm)")))
                except KeyError as ke: fig_polar.update_layout(title_text='Polar Veri Formatı Hatası')
            fig_polar.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, max_plot_dist]), angularaxis=dict(direction="counterclockwise", ticksuffix="°"))) # Göreli açılar için CCW daha mantıklı olabilir
            fig_time_layout = go.Layout(title_text='Zaman Serisi - Mesafe ' + title_suffix, xaxis_title="Zaman", yaxis_title="Mesafe (cm)", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(248,248,248,0.95)'); fig_time = go.Figure(layout=fig_time_layout)
            if not df_points.empty and 'timestamp' in df_points.columns:
                try:
                    ...
                    if not df_valid_time.empty: df_valid_time_sorted = df_valid_time.sort_values(by='timestamp'); datetime_series = pd.to_datetime(df_valid_time_sorted['timestamp'], unit='s'); fig_time.add_trace(go.Scatter(x=datetime_series, y=df_valid_time_sorted['mesafe_cm'], mode='lines+markers', name='Mesafe (cm)')); fig_time.update_xaxes(type='date', tickformat='%H:%M:%S')
                except KeyError as ke: fig_time.update_layout(title_text='Zaman Serisi Veri Formatı Hatası')
            return fig_map, fig_polar, fig_time
        except Exception as e: print(f"Grafik oluşturma sırasında genel hata: {e}"); error_fig.update_layout(title_text=f'Grafik Oluşturma Hatası: {str(e)[:30]}'); return error_fig, error_fig, error_fig
    ...
    def update_realtime_values_from_store(stored_data):
        ...
        try:
            if stored_data and stored_data.get('points') and len(stored_data['points']) > 0:
                last_point = stored_data['points'][-1]; angle_val = last_point.get('angle_deg'); dist_val = last_point.get('mesafe_cm'); speed_val = last_point.get('hiz_cm_s')
                angle_str = f"{angle_val:.0f}°" if pd.notnull(angle_val) else "--°"; distance_str = f"{dist_val:.1f} cm" if pd.notnull(dist_val) else "-- cm"; speed_str = f"{speed_val:.1f} cm/s" if pd.notnull(speed_val) else "-- cm/s"
        except IndexError: print("update_realtime_values: Boş 'points' listesi.")
        except KeyError as ke: print(f"update_realtime_values: Anahtar hatası: {ke}")
        except Exception as e: print(f"Anlık değerler güncellenirken genel hata: {e}")
    ...
    def update_analysis_panel_from_store(stored_data):
        ...
        info = stored_data.get('info'); scan_status = info.get('status') if isinstance(info, dict) else None
        if isinstance(info, dict) and info:
            if scan_status == 'running': area_str, perimeter_str, width_str, depth_str = "Tarama Sürüyor", "Tarama Sürüyor", "Tarama Sürüyor", "Tarama Sürüyor"
            elif scan_status == 'completed_analysis':
                area_val = info.get('hesaplanan_alan_cm2'); perimeter_val = info.get('cevre_cm'); width_val = info.get('max_genislik_cm'); depth_val = info.get('max_derinlik_cm')
                area_str = f"{area_val:.2f} cm²" if pd.notnull(area_val) else "Hesaplanmadı"; perimeter_str = f"{perimeter_val:.2f} cm" if pd.notnull(perimeter_val) else "Hesaplanmadı"; width_str = f"{width_val:.2f} cm" if pd.notnull(width_val) else "Hesaplanmadı"; depth_str = f"{depth_val:.2f} cm" if pd.notnull(depth_val) else "Hesaplanmadı"
            elif scan_status in ['completed_insufficient_points', 'completed_analysis_error', 'completed']: area_str, perimeter_str, width_str, depth_str = "Hesaplanmadı", "Hesaplanmadı", "Hesaplanmadı", "Hesaplanmadı"
            elif not scan_status and stored_data.get('scan_id'): area_str, perimeter_str, width_str, depth_str = "Durum Belirsiz", "Durum Belirsiz", "Durum Belirsiz", "Durum Belirsiz"
        elif stored_data.get('error'): error_msg_short = str(stored_data.get('error'))[:20]; area_str, perimeter_str, width_str, depth_str = f"Hata: {error_msg_short}", "Hata", "Hata", "Hata"
    ...
    def update_data_table_from_store(stored_data):
        if not stored_data or not stored_data.get('points'): return [], [{"name": "Veri Yok / Hata", "id": "placeholder"}]
        try:
            df_table = pd.DataFrame(stored_data['points'])
            if df_table.empty: return [], [{"name": "Tarama Noktası Yok", "id": "no_points"}]
            if 'timestamp' in df_table.columns: df_table['timestamp_str'] = pd.to_datetime(df_table['timestamp'], unit='s', errors='coerce').dt.strftime('%H:%M:%S.%f').str[:-3]
            else: df_table['timestamp_str'] = 'N/A'
        ...
        except Exception as e: print(f"Veri tablosu oluşturulurken hata: {e}"); return [], [{"name": f"Tablo Hatası: {str(e)[:30]}", "id": "table_error"}]

    @app.callback(
        Output('environment-estimation-text', 'children'),
        [Input('interval-component-main', 'n_intervals')],
        State('live-scan-data-store', 'data'),
        prevent_initial_call=True
    )
    def update_environment_estimation(n_intervals, stored_data):
        ...
        scan_info = stored_data.get('info'); points_data = stored_data.get('points'); scan_status = scan_info.get('status') if isinstance(scan_info, dict) else None
        ...
        try:
            ...
            if pd.notnull(max_g) and pd.notnull(max_d) and max_d > 0.01:
                ...
                if 0.8 < aspect_ratio < 1.25: shape_guess = "Oda benzeri/Geniş alan."
                elif aspect_ratio >= 1.25: shape_guess = "Geniş koridor/Yayvan engel."
                else: shape_guess = "Dar koridor/Uzun engel."
                ...
                if scipy_available and simplification_available and points_data and len(points_data) >= 3:
                    ...
                    if {'x_cm', 'y_cm', 'mesafe_cm'}.issubset(df_points_for_hull.columns):
                        ...
                        if len(df_valid_hull) >= 3:
                            try: points_np = df_valid_hull.values; hull = ConvexHull(points_np); shape_guess += f" Sınır: {len(hull.vertices)} köşe, Alan: {hull.area:.0f}cm²."
                            except scipy.spatial.qhull.QhullError as qe: print(f"Convex Hull (Qhull) hatası: {qe}"); shape_guess += " (Sınır analizi hatası.)"
                            except Exception as e_hull: print(f"Convex Hull genel hatası: {e_hull}"); shape_guess += " (Sınır analizi hatası.)"
                        else: shape_guess += " (Sınır için yetersiz nokta.)"
                    else: shape_guess += " (Sınır için veri eksik.)"
                elif not scipy_available or not simplification_available: shape_guess += " (Gerekli kütüphane eksik.)"
            elif scan_status == 'completed_analysis': shape_guess = "Analiz verisi (G/D) eksik."
        except KeyError as ke: print(f"Şekil tahmini anahtar hatası: {ke}"); shape_guess = f"Tahmin veri hatası."
        except Exception as e: print(f"Şekil tahmini genel hata: {e}"); shape_guess = f"Tahmin genel hata."
    ...
    def handle_stop_scan_script(n_clicks_stop):
        ...
        if pid_to_kill and is_process_running(pid_to_kill):
            try:
                ...
                if is_process_running(pid_to_kill): print(f"Betik (PID: {pid_to_kill}) SIGTERM'e yanıt vermedi, SIGKILL gönderiliyor..."); os.kill(pid_to_kill, signal.SIGKILL); time.sleep(0.5)
                msg_suffix = ""
                if os.path.exists(PID_FILE_PATH_FOR_DASH):
                ...
                if not is_process_running(pid_to_kill): return dbc.Alert(f"Betik (PID: {pid_to_kill}) başarıyla durduruldu.{msg_suffix}", color="info", duration=5000)
                else: return dbc.Alert(f"Betik (PID: {pid_to_kill}) durdurulamadı!{msg_suffix}", color="danger", duration=5000)
            except ProcessLookupError: return dbc.Alert(f"Betik (PID: {pid_to_kill}) zaten çalışmıyor.", color="warning", duration=4000)
            except Exception as e: return dbc.Alert(f"Betik (PID: {pid_to_kill}) durdurulurken hata: {e}", color="danger", duration=5000)
        else:
    ...
    def update_system_card(n_intervals):
        ...
        if pid and is_process_running(pid): script_status_text, status_class_name = f"Çalışıyor (PID: {pid})", "text-success"
        else:
    ...
    def export_csv_callback(n_clicks, stored_data):
        ...
        if latest_id:
            ...
            except (sqlite3.Error, pd.io.sql.DatabaseError) as e: print(f"CSV indirme DB hatası: {e}")
            except Exception as e: print(f"CSV indirme genel hata: {e}")
            finally:
                if conn: conn.close()
        return no_update
    ...
    def export_excel_callback(n_clicks, stored_data):
        ...
        if latest_id:
            try:
                df_points = pd.read_sql_query(f"SELECT * FROM scan_points WHERE scan_id = {latest_id} ORDER BY id ASC", conn); df_scan_info = pd.read_sql_query(f"SELECT * FROM servo_scans WHERE id = {latest_id}", conn)
            ...
            except (sqlite3.Error, pd.io.sql.DatabaseError) as e: print(f"Excel indirme DB hatası: {e}")
            except Exception as e: print(f"Excel indirme genel hata: {e}")
            finally:
                if conn: conn.close()
        return no_update
else:
