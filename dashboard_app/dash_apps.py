...
system_card = dbc.Card([  # Sistem durumu için ayrı ID'li bileşenler
    ...
    dbc.CardBody([
        dbc.Row([dbc.Col(html.Div([html.H6("Sensör Betiği:"), html.H5(id='script-status', children="Beklemede")]))],
                className="mb-2"),  # className buraya
        dbc.Row([
...
def update_all_graphs(n_intervals):
    ...
    fig_map = go.Figure().update_layout(title_text='2D Kartezyen Harita (Yükleniyor...)', uirevision=ui_revision_key,
                                        plot_bgcolor='rgba(248,248,248,0.95)')
    fig_polar = go.Figure().update_layout(title_text='Polar Grafik (Yükleniyor...)', uirevision=ui_revision_key,
                                          plot_bgcolor='rgba(248,248,248,0.95)')
    fig_time = go.Figure().update_layout(title_text='Zaman Serisi - Mesafe (Yükleniyor...)', uirevision=ui_revision_key,
                                         plot_bgcolor='rgba(248,248,248,0.95)')
    if error_msg_conn and not conn: titles = [f'2D Harita (DB Hatası)', f'Polar Grafik (DB Hatası)',
                                              f'Zaman Serisi (DB Hatası)']; [fig.update_layout(title_text=title) for
                                                                             fig, title in
                                                                             zip([fig_map, fig_polar, fig_time],
                                                                                 titles)]; return fig_map, fig_polar, fig_time
    if not id_to_plot:
    ...
    if conn:
        try:
            ...
            df_points = pd.read_sql_query(
                f"SELECT angle_deg, mesafe_cm, x_cm, y_cm, timestamp FROM scan_points WHERE scan_id = {id_to_plot} ORDER BY angle_deg ASC",
                conn)  # angle_deg'e göre sırala
        except Exception as e:
...
def update_analysis_panel_numeric(n_intervals):  # Fonksiyon adı değişebilir, shape'den ayırmak için
    ...
    if error_msg_conn:
        ...
        try:
            ...
            print(
                f"Analiz paneli DB sorgu hatası: {e}"); area_str, perimeter_str, width_str, depth_str = "Hata", "Hata", "Hata", "Hata"
        finally:
            if conn: conn.close()
...
def estimate_shape_callback(n_clicks):
    ...
    if conn:
        try:
            ...
            if latest_id:
                df_scan_info = pd.read_sql_query(
                    f"SELECT scan_extent_angle_setting, max_genislik_cm, max_derinlik_cm FROM servo_scans WHERE id = {latest_id}",
                    conn)
                df_points_for_hull = pd.read_sql_query(
                    f"SELECT x_cm, y_cm FROM scan_points WHERE scan_id = {latest_id} AND mesafe_cm > 0.1 AND mesafe_cm < 200 AND x_cm IS NOT NULL AND y_cm IS NOT NULL",
                    conn)
                if not df_scan_info.empty and not df_points_for_hull.empty and len(df_points_for_hull) >= 3:
                    ...
                    scan_extent = df_scan_info['scan_extent_angle_setting'].iloc[0] if pd.notnull(
                        df_scan_info['scan_extent_angle_setting'].iloc[0]) else DEFAULT_UI_SCAN_EXTENT_ANGLE
                    if pd.notnull(max_g) and pd.notnull(max_d) and max_d > 0:
                        aspect_ratio = max_g / max_d
                        if abs(scan_extent * 2 - 270) < 30:
                            shape_guess = "Geniş Açılı Alan (Oda?)" if 0.7 < aspect_ratio < 1.4 else (
                                "Geniş ve Yayvan" if aspect_ratio >= 1.4 else "Dar ve Uzun")
                        elif abs(scan_extent * 2 - 180) < 30:
                            shape_guess = "Yarım Daire/Sektör" if 0.7 < aspect_ratio < 1.4 else "Genişleyen Sektör"
                        else:
                            shape_guess = "Belirli Sektör"
                        try:
                            points_np = df_points_for_hull[['y_cm', 'x_cm']].values;
                        ...
                        shape_guess = "Temel analiz verisi eksik."
                else:
                    shape_guess = "Şekil analizi için yeterli nokta/tarama bilgisi yok."
            else:
                shape_guess = "Analiz edilecek tarama bulunamadı."
        except Exception as e:
            print(f"Şekil tahmini hatası: {e}"); shape_guess = "Şekil tahmini sırasında hata."
        finally:
            if conn: conn.close()
    return shape_guess
