...
def is_process_running(pid):
    try:
        return pid is not None and isinstance(pid, int) and pid > 0 and psutil.pid_exists(pid)
    except (TypeError, psutil.Error) as e:
...
def get_latest_scan():
    ...
    try:
        return (Scan.objects.filter(status='running').first() or
                Scan.objects.select_related().order_by('-start_time').first())
...
def handle_start_scan_script(n_clicks, duration, step, buzzer, invert, steps_rev):
    try:
        ...
        try:
            ...
            logger.error(f"Sensör betiği başlatma hatası: {e}")
            return dbc.Alert(f"Hata: {str(e)}", color="danger")
    except Exception as e:
        logger.error(f"Sensör betiği başlatma hatası: {e}")
        return dbc.Alert(f"Hata: {str(e)}", color="danger")
...
def handle_stop_scan_script(n_clicks):
    if n_clicks == 0:
        return no_update
    try:
        # Cleanup işlemleri bir fonksiyona taşınmalı
        cleanup_scan_resources()
        return dbc.Alert("İşlem başarıyla durduruldu.", color="success")
    except Exception as e:
        return dbc.Alert(f"Hata: {str(e)}", color="danger")
...
def update_system_card(n):
    ...
    if os.path.exists(SENSOR_SCRIPT_PID_FILE):
        try:
            with open(SENSOR_SCRIPT_PID_FILE, 'r') as pf:
                pid = int(pf.read().strip())
        except (IOError, ValueError) as e:  # Specific exceptions
            print(f"PID dosyası okuma hatası: {e}")
            pid = None
...
def validate_scan_params(duration, step, buzzer, steps_rev):
    try:
        ...
        if not (10 <= duration <= 720):
            raise ValueError("Tarama açısı 10-720 derece arasında olmalı")
        if not (0.1 <= abs(step) <= 45):
            raise ValueError("Adım açısı 0.1-45 arasında olmalı")
        if not (0 <= buzzer <= 200):
            raise ValueError("Buzzer mesafesi 0-200 cm arasında olmalı")
        if not (500 <= steps_rev <= 10000):
            raise ValueError("Motor adımı 500-10000 arasında olmalı")
